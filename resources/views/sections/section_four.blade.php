<div class="row vh100">
    <div class="col-12 col-md-12 padding-unset">
        <div id="carousel_antigua" class="carousel slide" data-ride="carousel">
            <div class="carousel-inner">
                <div class="carousel-item active">
                    <div class="carousel-image"
                         style="background-image: url('/images/antigua_images/antigua_five.jpg')"></div>
                    <div class="carousel-caption d-none d-md-block">
                        <div class="center">
                            <h1 class="text-light text_shadow_primary"><strong>ANTIGUA Y SUS ALREDEDORES </strong></h1>
                            <h2 class="text-light text_shadow_primary"></h2>
                        </div>
                    </div>
                </div>
                <div class="carousel-item">
                    <div class="carousel-image"
                         style="background-image: url('/images/antigua_images/antigua_one.jpg')"></div>
                    <div class="carousel-caption d-none d-md-block">
                        <h1 class="text-light text_shadow_primary"><strong>ANTIGUA Y SUS ALREDEDORES</strong></h1>
                        <h2 class="text-light text_shadow_primary"></h2>
                    </div>
                </div>
                <div class="carousel-item">
                    <div class="carousel-image"
                         style="background-image: url('/images/antigua_images/antigua_eleven.jpg')"></div>
                    <div class="carousel-caption d-none d-md-block">
                        <h1 class="text-light text_shadow_primary"><strong>ANTIGUA Y SUS ALREDEDORES</strong></h1>
                        <h2 class="text-light text_shadow_primary"></h2>
                    </div>
                </div>
                <div class="carousel-item">
                    <div class="carousel-image"
                         style="background-image: url('/images/antigua_images/antigua_ten.jpg')"></div>
                    <div class="carousel-caption d-none d-md-block">
                        <h1 class="text-light text_shadow_primary"><strong>ANTIGUA Y SUS ALREDEDORES</strong></h1>
                        <h2 class="text-light text_shadow_primary"></h2>
                    </div>
                </div>
            </div>
            <a class="carousel-control-prev" href="#carousel_antigua" role="button" data-slide="prev">
                <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                <span class="sr-only">Anterior</span>
            </a>
            <a class="carousel-control-next" href="#carousel_antigua" role="button" data-slide="next">
                <span class="carousel-control-next-icon" aria-hidden="true"></span>
                <span class="sr-only">Siguiente</span>
            </a>
        </div>
    </div>
</div>
